...
export const ja: Messages = {
  ...
  settings: {
    ...
    guard: {
      title: `ID ガード`,
      intro: `アカウントのフォルダーが受け持たないすべてに効く、マシン全体の設定です。フォルダーは各アカウントのページで割り当てます。この 2 つのスイッチは、そうしたフォルダーの外で有効なプロフィールが git の既定の ID を提供するかどうかと、すべてのコミットをそれが作られたフォルダーのアカウントと照合するかどうかを決めます。`,
  ...
  form: {
    ...
    savingWalk: `{folder} でリポジトリを検索中`,
    savingScan: `{folder}: リポジトリ {total} 件中 {done} 件`,
    savingApply: `git のルールを適用中...`,
